...
for (let i = 0; i < btn.length; i++) {
  btn[i].addEventListener("click", (ev) => {
    ...
    //console.log(process.processedImage)
...
var process = {
  ...
  removeSingle: function (item){
    let arrCopy = this.processedImage;
    for(let [ix, xterm] of arrCopy.entries()){
      console.log("we entered")
      if(ix == 0)
        continue;
      this.processedImage.splice(1);
    }
    return true;
  },

  redoAll: function (item){
    for(let [ix, xterm] of item.entries()){
      console.log("we entered redo")
      if(ix == 0)
        continue;
      this[arrCopy[ix]["type"]](this.lastImage());
    }
    return true;
  },
    ...
    if (this.checkList(item)) {      
      for (let [index, x] of this.processedImage.entries()) {
        if (x.type === item.type) {
          ...
          if(this.processedImage.length > 2){
            //remove item first
            this.processedImage.splice(index, 1);
            //copy the new list
            this.storeItem(this.getImages())
            const newList = this.getStoredImages();
            //reset old list
            this.processedImage.splice(1);
            console.log(this.processedImage)
            //run new list of filter skipping the first
            for(let [i, iItem] of newList.entries()){
              //skip first item
              if (i == 0)
                continue;
              console.log(newList[i]["type"])
              setTimeout(()=>{this[newList[i]["type"]](this.lastImage())},500);
            }
            console.log("we entered many")
            this.hideLoader()
            break;
            //test and see
          }else{
            //we only have the original and one filter
            console.log("we entered last")
            this.processedImage.splice(index, 1);
            this.storeItem(this.processedImage)
...
function batch(val){
    ...
    reader.addEventListener(
        ...
        function (e) {
            ...
            document.getElementById("loader").classList.add("enforce");
            ...
            img.onload = function () {
                ...
                process.hideLoader();
                document.getElementById("loader").classList.remove("enforce");
            };
